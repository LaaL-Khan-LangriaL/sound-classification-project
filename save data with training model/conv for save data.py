...
import pickle  ######### for loading binary files #####
from keras.callbacks import ModelCheckpoint  ####for saving model #####3
from config import config
 


#### for check data if existing files in pickle folder ######
def check_data():
...
def build_rand_feat():
    ...
    with open(config.p_path, 'wb') as handle:
         pickle.dump(config, handle ,protocol=2)   ##for python 2 prtcl is 2####
    return x, y
...
###### grab files and read csv files from folder ##########
...
config = config(mode='conv')   ##### this is with conv model for first chk   ######
